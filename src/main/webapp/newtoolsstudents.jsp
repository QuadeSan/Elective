...
<c:set var="lastpage" value="toolsstudents" scope="session" />
    ...
    <div class="tools">
        ...
        <c:if test="${allStudentsList != null && not empty allStudentsList}">
          ...
          <a href="pagination?current-page=${currentPage -1}"> <button id="prevbtn"> <fmt:message key='tools_students.previous.button'/> </button> </a>
          </c:if>
          <c:if test="${currentPage lt pageCount}">
          <a href="pagination?current-page=${currentPage +1}"> <button id="nextbtn"> <fmt:message key='tools_students.next.button'/> </button> </a>
          </c:if>
       </c:if>
    </div>
    </div>
